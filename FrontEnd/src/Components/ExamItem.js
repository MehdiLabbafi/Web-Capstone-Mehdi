                ...
                props.children.map(data => {
                    ...
                    return (
                        <div key={v4()} className='w-full mb-6 h-32 md:h-24 block md:flex flex-wrap py-2 items-center justify-between rounded-lg px-4 bg-gray-400 dark:bg-gray-600 bg-opacity-25 dark:bg-opacity-40 shadow-md shadow-gray-500 dark:shadow-bg_dark-100'>
                            ...
                            <div>
                                {
                                    <p className='dark:text-gray-500 text-bg_dark-50 text-sm'> {data[4]} <span>up to </span> {data[5]}  {gregorian_to_jalali(Number(year), Number(month), Number(date))[0] + "/" + gregorian_to_jalali(Number(year), Number(month), Number(date))[1] + "/" + gregorian_to_jalali(Number(year), Number(month), Number(date))[2]} </p>
                                }
                            </div>
                        </div>
                    );
