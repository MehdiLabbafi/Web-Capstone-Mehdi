...
const Signup = () => {
    ...
    useEffect(() => {
        document.title = BaseName + "sign up";
    }, []);
        ...
        if (!nameFinal && !familyFinal && !mobileFinal && !usernameFinal && !passwordFinal && nameActive && familyActive && mobileActive && usernameActive && passwordActive && name.length && family.length && username.length && mobile.length && password.length) {
            if (mobile.match(mobileValidat)) {
                if (username.match(usernameValidate)) {
                    ...
                    axios.post(BaseURL + "cms-backend/signup.php", {
                        ...
                        .then(function (response) {
                            if (response.data == "200") {
                                Toastify_Success("Registration was successful");
                                setisSignup(false);
                                navigate("/Login")

                            } else if (response.data.username == username) {
                                Toastify_Error(`Already exists ${response.data.username} Username`);
                                setisSignup(false);
                            }
                            else {
                                Toastify_Error("An error occurred during the operation !");
                                setisSignup(false);
                            }
                        })
                        .catch(function (e) {
                    ...
                    Toastify_Error("Username is invalid");
                    setUsername("");
                    setPassword("");
                    setisSignup(false);
                }
            } else {
                Toastify_Error("The mobile number is invalid");
                setMobile("");
                setPassword("");
                setisSignup(false);
            }
            ...
            Toastify_Error("Please fill in the fields");
            setisSignup(false);
        }
    }
        ...
        <div className='w-screen  px-4 py-4 md:py-0 md:px-0 h-screen flex items-start font-Shabnam '>
            <div className='w-240 pb-8 xl:pb-0 dark:bg-bg_dark-50 m-auto rounded-2xl shadow-lg bg-bg_light-100 dark:shadow-slate-900 flex'>
                ...
                <div className='w-full xl:w-1/2 h-full rounded-r-2xl' dir='rtl'>
                    <h2 className='font-Shabnam_Bold text-3xl text-center text-gray-600 dark:text-gray-400 my-16'><span className='text-green-600'>sign up</span> In the system</h2>
                    <form className='dark:text-gray-400 text-gray-600 mx-14 m-auto select-none'>
                        <label htmlFor="name" className={'block mb-2 ' + (nameActive && name && (nameFinal ? " text-red-600 " : " text-green-500 ")) + (isSignup ? " pointer-events-none" : "pointer-events-auto")}> First Name </label>
                        ...
                        <label htmlFor="family" className={'block mb-2 mt-5 ' + (familyActive && family && (familyFinal ? " text-red-600 " : " text-green-500 ")) + (isSignup ? " pointer-events-none" : "pointer-events-auto")}> Last Name </label>
                        ...
                        <label htmlFor="mobile" className={'block mb-2 mt-5 ' + (mobileActive && mobile && (mobileFinal ? " text-red-600 " : " text-green-500 ")) + (isSignup ? " pointer-events-none" : "pointer-events-auto")}> Mobile Phone </label>
                        ...
                        <label htmlFor="username" className={'block mb-2 mt-5 ' + (usernameActive && username && (usernameFinal ? " text-red-600 " : " text-green-500 ")) + (isSignup ? " pointer-events-none" : "pointer-events-auto")}> Username </label>
                        ...
                        <label htmlFor="password" className={'block mb-2 mt-5 ' + (passwordActive && password && (passwordFinal ? " text-red-600 " : " text-green-500 ")) + (isSignup ? " pointer-events-none" : "pointer-events-auto")}> Password </label>
                        ...
                        <div className='flex justify-between mt-12 mb-12'>
                            <button type='button' onClick={signup} className={'w-full bg-green-600 text-white text-center ml-3 py-1 rounded-md hover:bg-green-500 transition-all flex items-center justify-around ' + (isSignup ? "pointer-events-none" : "pointer-events-auto")}><p className={'text-center m-auto ' + (isSignup && "hidden")}>sign up</p> <img className={'w-6 absolute animate-spin ' + (!isSignup && " hidden")} src={loading} alt="loading" /></button>
                            <Link type='button' className='w-full border-2 text-green-600 border-green-600 text-center mr-3 py-1 rounded-md hover:border-green-500 transition-all' to={"/Login"}>Login</Link>
                        </div>
                    </form>
                </div>
