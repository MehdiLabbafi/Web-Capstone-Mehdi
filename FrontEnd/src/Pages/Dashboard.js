...
const Dashboard = () => {
    ...
    useEffect(() => {
        getData();
        document.title = BaseName + "User Account";
    }, []);
                ...
                <div className='width-set h-full absolute left-0 py-2 pt-28 sm:pt-28 2xl:pt-9 sm:py-9 px-2 sm:px-8 overflow-y-auto' dir='rtl' >
                    <div className=' w-full mb-9 h-72 md:h-48 dark:bg-bg_dark-50 text-text_light-100  bg-bg_light-100 rounded-xl flex justify-between' dir='rtl'>
                        <div className='p-5 dark:text-texts'>
                            <p className='font-Shabnam_Medium  text-xl'>🎉 Welcome to your account </p>
                            <p className='font-Shabnam_Thin text-gray-500 mr-8 mt-4'>We are very honored to host you in the Online Schools system .</p>
                            <p className='font-Shabnam_Medium tracking-wide mt-2 text-actives mr-8'>With <span className='font-Shabnam_Bold'>{userCount}</span>  Active user</p>
                            <Link to={"#"} className='inline-block  font-Shabnam_Bold mr-8 mt-4 bg-btns px-10 py-1.5 rounded-lg transition-all shadow-lg hover:shadow-tags text-texts'>Blog</Link>
                        </div>
                    ...
                    <div className="flex flex-row-reverse flex-wrap xl:flex-nowrap mb-9 xl:mb-0">
                        <div className='xl:mr-6 xl:w-2/3 w-full h-105 md:h-48 dark:bg-bg_dark-50 mb-9 bg-bg_light-100 rounded-xl p-6 text-texts font-Shabnam ' dir='rtl'>
                            <h3 className='dark:text-gray-400 text-text_light-100 text-xl'>Information</h3>
                            <div className='p-11 justify-between block md:flex'>
                                <ItemUser context={field} content="Field" bg_color="bg-green-300" text_color="text-green-500" dPath="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                                <ItemUser context={grade} content="Grade" bg_color="bg-yellow-300" text_color="text-yellow-500" dPath="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                                <ItemUser context={userClass} content="Class" bg_color="bg-red-300" text_color="text-red-500" dPath="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" />
                                <ItemUser context={groups} content="Groups" bg_color="bg-purple-300" text_color="text-purple-500" dPath="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                            </div>
                        </div>
                        ...
                        <div className='xl:w-1/3 w-full h-96 md:h-48 xl:mt-0 mb-9 md:mb-0' dir='rtl'>
                            <div className='w-full h-48 rounded-xl  flex flex-wrap md:flex-nowrap justify-between mb-9 xl:ml-3' dir='rtl'>
                                <div className='md:w-1/2 w-full h-full dark:bg-bg_dark-50 bg-bg_light-100 mb-9 md:mb-0 md:ml-2 rounded-xl flex'>
                                    <h3 className='dark:text-gray-400 text-text_light-100 font-Shabnam_Thin px-5 pt-5 pb-2 absolute'>Homeworks</h3>
                                    <Gauges opacity={localStorage.getItem("theme") == "dark" ? .15 : 1} title={"Completed"} size={"45%"} series={homeworkStatus && !isNaN(Number(homeworkStatus)) ? Number(homeworkStatus) / Number(homeworkCount) * 100 : 0} color={"#28C76F"} />
                                </div>
                                <div className='md:w-1/2 w-full h-full dark:bg-bg_dark-50 bg-bg_light-100  rounded-xl  md:mr-2 flex'>
                                    <h3 className='dark:text-gray-400 text-text_light-100 font-Shabnam_Thin px-5 pt-5 pb-2 absolute'>Study Hours</h3>
                                    <Gauges opacity={localStorage.getItem("theme") == "dark" ? .15 : 1} title={"Study"} size={"45%"} series={totalStudyTime && mustStudy && !isNaN(Number(totalStudyTime)) && !isNaN(Number(mustStudy)) ? totalStudyTime / mustStudy * 100 : 0} color={"#655CD4"} />
                                </div>
                    ...
                    <div className="flex flex-row-reverse flex-wrap xl:flex-nowrap">
                        <div className='w-full xl:w-2/3 h-105 dark:bg-bg_dark-50 bg-bg_light-100  rounded-xl mr-0 xl:mr-6 p-6 text-texts font-Shabnam pb-9' dir='rtl'>
                            <h3 className='dark:text-gray-400 text-text_light-100 font-Shabnam_Thin absolute'>Today's Class Schedule</h3>
                                            ...
                                            row.push(
                                                ...
                                                <div key={v4()} className='w-full h-16 flex items-center justify-between'>
                                                    <div className='flex'>
                                                        ...
                                                        <p className='hidden sm:block text-center font-Shabnam_Thin dark:text-gray-400 text-text_light-100 border-2 border-gray-300 dark:border-gray-800 shadow-md dark:shadow-slate-800 px-4 py-1 rounded-xl min-set '>{time_start + "  To  " + time_end}</p>
                                                    </div>
                                                    <a target="_blank" href={checkClassStatus ? class_link : "#"} className={'flex hover:text-green-400  transition-all dark:text-texts   dark:hover:text-green-400 ' + (checkClassStatus ? "pointer-events-auto text-text_light-100" : "pointer-events-none dark:text-gray-500 text-gray-300")} >Join the session
                        ...
                        <div className='w-full xl:w-1/3 h-105 font-Shabnam flex flex-wrap md:flex-nowrap xl:block  xl:mt-0 mt-9'>
                            <div className='w-full h-48 dark:bg-bg_dark-50 bg-bg_light-100  rounded-xl mb-9 md:ml-2 xl:ml-0' dir='rtl'>
                                <h3 className='dark:text-gray-400 text-text_light-100 font-Shabnam_Thin px-5 pt-5 pb-2 absolute'>Exam Results</h3>
                                {examResult && Array.isArray(examResult) &&
                                    <LineChart max={100} curv="straight" colors="#685FD7" name={" Exam "} height={190} paddingLeft={40} data={examResult} cat={examTime} />
                                }
                            </div>
                            <div className=' w-full h-48 dark:bg-bg_dark-50 bg-bg_light-100  rounded-xl md:mr-2 xl:mr-0' dir='rtl'>
                                <h3 className='dark:text-gray-400 text-text_light-100 font-Shabnam_Thin px-5 pt-5 pb-2 absolute'>Studies</h3>
                                {studys && Array.isArray(studys) &&
                                    <LineChart max={undefined} curv="smooth" colors="#EA5455" name={"Study "} height={190} paddingLeft={40} data={studys} cat={studyTime} />
                                }
                            </div>
                        </div>
