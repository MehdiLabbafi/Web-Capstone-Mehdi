...
const Study = () => {
    ...
    const sendStudy_Hr = (studyHr, lessonUs) => {
        ...
        if (id && studyHr && lessonUs) {
            axios.post(BaseURL + "cms-backend/studyHrInsert.php", {
                ...
                .then(function (response) {
                    if (response.data == "200") {
                        Toastify_Success("Study hour successfully registered")
                        setlesson("");
                        setStudyTime("");
                        setIsLoading(false)
                    } else {
                        Toastify_Error("An error occurred while sending.")
                        setlesson("");
                        setStudyTime("");
                        setIsLoading(false)
                    }
            ...
            Toastify_Info("Please fill in the fields")
            setlesson("");
            setStudyTime("");
            setIsLoading(false)
        }
            ...
            <div className='flex w-full h-full'>
                ...
                <div className='width-set h-full absolute left-0  py-2 pt-28 sm:pt-28 2xl:pt-9 sm:py-9 px-2 sm:px-8 font-Shabnam ' >
                    <div className='w-full mb-9 h-115 sm:h-80 dark:bg-bg_dark-50 rounded-xl bg-bg_light-100' dir='rtl'>
                        <h3 className='text-gray-400 absolute  font-Shabnam_Bold text-xl p-6'>Study time registration</h3>
                        <form className='block sm:flex justify-between w-full h-full pt-28 px-16 font-Shabnam'>
                            <div className='w-full sm:w-1/2 mx-2 sm:mx-8'>
                                ...
                                <div className="mt-9 relative z-0 mb-6 group w-full">
                                    ...
                                    <label htmlFor="study_hr" className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-80 top-3 -z-10 origin-[0] peer-focus:right-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-80 peer-focus:-translate-y-6">measure Study</label>
                                </div>
                            </div>
                            ...
                            <div className='w-full sm:w-1/2 mx-2 sm:mx-10'>
                                <div className="mb-5 relative z-0 w-full group">
                                    ...
                                    <label htmlFor="subject" className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-80 top-3 -z-10 origin-[0] peer-focus:right-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-80 peer-focus:-translate-y-6">Subject Study</label>
                                </div>
                                <div className="mt-9 relative z-0 mb-6 group w-full">
                                    <input max={200}
                                        type="number" name="test" className=" input-set block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " autoComplete='off' />
                                    <label htmlFor="test" className="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-80 top-3 -z-10 origin-[0] peer-focus:right-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-80 peer-focus:-translate-y-6">Number of tests</label>
                                </div>
                            </div>
                        </form>
                    ...
                    <div className='w-full mb-9 p-6 dark:bg-bg_dark-50 bg-bg_light-100 rounded-xl ' dir='rtl'>
                        <h3 className='text-gray-400  font-Shabnam_Bold text-xl'>Study pattern</h3>
