            ...
            axios.post(BaseURL + "cms-backend/login.php", {
                ...
                .then(function (response) {
                    if (response.data.resp == "200") {
                        ...
                        Toastify_Success("Successfully logged in");
                        setisLogging(false);
                        ...
                        Toastify_Error("Your account is currently inactive");
                        setisLogging(false);
                    } else {
                        Toastify_Error("The username or password is incorrect");
                        setisLogging(false);
                    }
                })
                .catch(function (err) {
                    Toastify_Error("An error occurred during login")
                    setisLogging(false);
                })
        } else {
        ...
        <div className='w-screen h-screen px-4 py-4 md:py-0 md:px-0 flex items-start font-Shabnam '>
            <div className='w-225 pb-12 xl:pb-0 h-137.5 dark:bg-bg_dark-50 m-auto rounded-2xl shadow-lg bg-bg_light-100 dark:shadow-slate-900 flex'>
                <div className='w-full xl:w-1/2 h-full rounded-l-2xl' dir='rtl'>
                    <h2 className='font-Shabnam_Bold text-3xl text-center text-gray-600 dark:text-gray-400 my-16'><span className='text-blue-700 font-Shabnam_Bold'>Login</span> To the system</h2>
                    <form className=' text-gray-600 dark:text-gray-400 mx-14 m-auto select-none'>
                        <label htmlFor="username" className={'block mb-3 ' + (isLogging ? " pointer-events-none " : " pointer-events-auto ") + (usernameActive && username && (usernameFinal ? "text-red-600" : "text-green-500"))} > Username </label>
                        ...
                        <label htmlFor="password" className={'block mb-3 mt-5  ' + (isLogging ? " pointer-events-none " : " pointer-events-auto ") + (passwordActive && password && (passwordFinal ? "text-red-600" : "text-green-500"))}> Password </label>
                        ...
                        <div className="block sm:flex justify-between pt-6">
                            <div className="flex items-start mb-7 sm:mb-0  ">
                                ...
                                <div className="ml-3 text-sm">
                                    <label htmlFor="remember" className={"font-medium text-gray-900 dark:text-gray-400 px-3 " + (isLogging ? " pointer-events-none" : "pointer-events-auto")}>Remember Me</label>
                                </div>
                            </div>
                            <Link to={"#"} className={'text-rose-500 ' + (isLogging ? " pointer-events-none" : "pointer-events-auto")}>Forgot Password ?</Link>
                        </div>

                        <div className='flex justify-between mt-12'>
                            <button onClick={doLogin} type='button' className={'w-full bg-blue-700 text-white text-center ml-3 py-1 rounded-md hover:bg-primary transition-all flex items-center justify-around ' + (isLogging ? " pointer-events-none" : "pointer-events-auto")}><p className={'text-center m-auto ' + (isLogging && "hidden")}>Login</p> <img className={'w-6 absolute animate-spin ' + (!isLogging && " hidden")} src={loading} alt="loading" /></button>
                            <Link type='button' className='w-full dark:text-blue-500 text-blue-700 border-2 border-blue-700 text-center mr-3 py-1 rounded-md hover:border-primary transition-all' to={"/Signup"}>Sign up</Link>
                        </div>
                    </form>
                </div>
